...
//            //Test RBTree
            ...
            //Test HashTableCyCQ
            startTime = System.nanoTime();
            HashTableCyCQ<String,Integer> hashTableCyCQ = new HashTableCyCQ<String, Integer>();
            for (String word:words){
                if (hashTableCyCQ.contains(word)){
                    hashTableCyCQ.set(word,hashTableCyCQ.get(word)+1);
                }else {
                    hashTableCyCQ.add(word,1);
                }
            }

            for (String word:words){
                hashTableCyCQ.contains(word);
            }
            endTime = System.nanoTime();
            time = (endTime-startTime)/1000000000.0;
            System.out.println("HashTableCyCQ:"+time+"s");
        }
    }
}
